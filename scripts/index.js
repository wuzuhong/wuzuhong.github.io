    ...
    if (blogs != null && blogs.length > 0) {
        blogs.forEach(blog => {
            ...
            if(existTags[tag] == null){
                ...
                var element = '' +
                '<button type="button" class="btn btn-default btn-sm" onclick="searchBlogByTag($(this))" style="margin-top:5px;margin-left:5px" title="' + tagStr + '">' +
                tagStr +
                '</button>'
                $("#tags").append(element)
                existTags[tag] = tag
...
function searchBlogByTag(tagElement) {
    $("#tags").find("button").attr("class","btn btn-default btn-sm")
