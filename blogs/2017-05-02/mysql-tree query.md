...
* 第二步：将上一步查出来的子节点的id集合作为本次查找的父节点id集合，并查出属于这些父节点集合的一级子节点（注意：只查找一级子节点）
* 第三步：若第二步有值，则循环执行第二步，若第二步没有值，则退出循环。

#### 函数
```
DELIMITER $$
CREATE FUNCTION `testdb`.`getChildList`(parentId VARCHAR (21845)) RETURNS VARCHAR(21845) CHARSET utf8
  BEGIN
...
```
DELIMITER $$
CREATE FUNCTION `testdb`.`getParentList`(cid VARCHAR(21845)) RETURNS VARCHAR(21845) CHARSET utf8
  BEGIN
