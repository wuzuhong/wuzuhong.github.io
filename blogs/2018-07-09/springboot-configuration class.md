...
当然也支持在`@Configuration`注解下面使用`@ImportResource`注解来加载xml文件配置。其支持通配符，例如：`@ImportResource("classpath*:aabb/*-ccdd.xml")`。也支持自定义xml文件读取逻辑，只需要自定义一个`BeanDefinitionReader`的实现类，然后这样使用`@ImportResource(locations="classpath*:aabb/*-ccdd.xml", reader=MyXmlBeanDefinitionReader)`，最后实现这个`MyXmlBeanDefinitionReader`类：
```java
