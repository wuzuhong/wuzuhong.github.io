...
再举一个现实的例子，电脑上的USB接口，其实电脑通过USB接口就是为了获取数据，它不管这个数据是哪个设备上的，有可能是来自手机，也有可能是来自U盘，反正电脑只知道自己要的是数据，它不管这些数据的具体存储形式。
