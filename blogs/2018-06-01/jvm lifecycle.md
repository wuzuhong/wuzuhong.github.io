...
#### 栈（Stack）
栈，它是线程私有的，每个线程创建的同时都会创建栈。

用于存储当前线程中基本类型（java中定义的八种基本类型：boolean、char、byte、short、int、long、float、double）的数据、局部变量、部分的返回结果以及Stack Frame，非基本类型的对象在栈上仅存放一个指向堆上的地址。

#### 堆（Heap）
堆，它是线程共享的，所有线程使用的都是同一个堆。

**堆区包括：Young Gen（新生代）和Tenured Gen（旧生代）。Young Gen（新生代）又包括：Eden Space（伊甸园区）、Survivor Space（幸存者区）。Survivor Space（幸存者区）又包括：From区、To区**。

堆是JVM用来存储对象实例以及数组值的区域，可以认为Java中所有通过new创建的对象的内存都在此分配（非静态的全局变量也是作为对象的一部分，存储在堆中），堆中的对象的内存需要等待GC进行回收。
...
在Sun JDK中对方法区的实现方式为PermanetGeneration，即**持久代**。
